...
- Require `memory catalog show` plus direct inspection of relevant memory files with normal tools such as `rg`, `sed`, and `cat` when prior memory could affect final handoff or debrief accuracy.
