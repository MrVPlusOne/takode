...
- If the assignment, quest, or post-compaction recovery context points to relevant file-based memory, use an available preloaded catalog for orientation; run `memory catalog show` only when orientation is missing, incomplete, or a full current catalog is needed. Use `memory catalog diff` when freshness matters, inspect relevant memory files directly with normal tools such as `rg`, `sed`, and `cat`, and mention any memory files that materially affected the read-in.
...
- For an agreed replacement, check the quest and linked sources for unresolved coexistence assumptions under the shared Design Replacement guidance. Flag material scope or authority ambiguity; leave implementation and consumer investigation to Work.
- Surface facts that may justify a leader-owned Journey revision; do not assume approval for a different phase sequence.
- Distinguish access/authority safety from fidelity-changing payload transformation. Continue to protect credentials, permissions, destructive/external effects, source integrity, and shared resources, but do not infer from a protected access method alone that payload content needs sanitation. Flag a proposed transformation as a scope/authority ambiguity when it lacks explicit user direction, project/repository policy, an approved contract, or concrete payload evidence.
