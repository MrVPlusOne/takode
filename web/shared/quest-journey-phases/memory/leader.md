...
- Final `User review checks` are user-owned checks only. Memory should complete with an empty list when no user action remains, or with concise items only when the user still needs to inspect or do something after all agent-owned evidence is complete. Tests, reviews, Execute evidence, Port, push status, post-port verification, Memory closure, and other agent-owned evidence belong in phase docs, review verdicts, artifacts, Port notes, commit metadata, and the debrief, not in final checks.
