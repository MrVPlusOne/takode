...
- User Checkpoints are mandatory by default. Skip one only when the approved Journey already includes an explicit phase note for that User Checkpoint saying it is optional and giving the concrete skip condition, and the post-Explore decision records that the condition was satisfied.
