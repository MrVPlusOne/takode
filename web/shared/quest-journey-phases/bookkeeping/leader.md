...
Bookkeeping is for cross-phase or external durable state beyond normal phase notes: consolidated summaries, external docs or links, superseded facts, notification cleanup, thread cleanup, file-based memory updates, or shared-state updates. Do not dispatch Bookkeeping just to repeat the documentation a phase actor should already write, and do not use it for final User review check settlement or other final closure now owned by Memory.
...
- Use final Memory, not Bookkeeping, as the normal owner for final User review check settlement, final debrief metadata, and debrief TLDR before completing a non-cancelled quest.
