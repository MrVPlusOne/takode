...
- Run `memory catalog show` first, then inspect relevant existing memory files directly with normal tools such as `rg`, `sed`, and `cat` so the update works with existing memory instead of duplicating it.
