...
- If Explore may lead to implementation but findings could need user steering first, plan or revise to `EXPLORE -> USER_CHECKPOINTING -> IMPLEMENTING`. A User Checkpoint is mandatory unless the approved Journey has an explicit phase note saying it is optional with a concrete skip condition; after Explore, skipping still requires recording that the condition was satisfied.
