...
- Expect reviewers to start from the worker's tracked diff, phase note, and verification evidence before deciding whether to rerun checks. Do not ask for exact duplicate automated reruns when worker evidence is fresh, complete, tied to the unchanged reviewed commit, and no risk trigger exists; do ask for bounded reruns or probes when evidence is stale/incomplete, tests are the change, behavior is high-risk or environment-sensitive, prior failures or ambiguous results exist, command/result reporting is incomplete, or an independent probe materially reduces landing risk.
- Require tracked docs, instructions, tests, fixtures, generated templates, changelog, Playground coverage, or other tracked-artifact updates to be reviewed before acceptance when the change affects those surfaces. Missing tracked follow-up belongs back in Implement and Code Review, not in final Memory.
- Require reviewers to judge phase documentation quality, not just presence: phase relevance, useful full detail, TLDR completeness when appropriate, no routine raw commit/hash bookkeeping in TLDRs or user-facing summaries when exact identifiers are already carried elsewhere, and correct phase association when the phase-scoped primitive is available.
- Require reviewers to challenge proposed final `User review checks` that are really agent-owned evidence, such as tests, Code Review, Execute, Port, push, post-port verification, or Memory closure. Reviewers should cite concrete item text and route it back by judgment, not classifiers or heuristics.
