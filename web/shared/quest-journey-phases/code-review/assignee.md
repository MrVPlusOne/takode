...
- Treat proposed agent-owned final `User review checks` as quest-hygiene findings: tests, Code Review, Execute, Port, push, post-port verification, Memory closure, and other agent work belong in phase docs, review verdicts, artifacts, Port notes, commit metadata, and debrief context. Do not use a classifier or rewrite the checklist by guesswork; cite the concrete item text and route it back to the owner or final Memory when direct safe correction is unavailable.
- If the tracked change affects user-facing behavior, agent-facing workflow, CLI behavior, prompts, generated instructions, schemas, reusable procedures, docs, tests, fixtures, changelog, or Playground states, verify that the necessary tracked updates are included before acceptance. Missing tracked docs or instruction work is a Code Review finding routed back to Implement; do not defer it to final Memory.
- Review documentation quality, not just presence: it should be relevant to the phase, contain useful full detail, preserve major points in TLDR metadata when appropriate, keep routine raw commit/hash bookkeeping out of TLDRs and user-facing summaries when exact identifiers are already carried elsewhere, and be correctly phase-associated when the phase-scoped primitive is available.
...
- Do not become the implementer, porter, or redesign owner. You may directly fix only small quest-hygiene issues already supported by the workflow, such as stale addressed flags, refreshable summaries, or User review checks backed by evidence.
...
- Document the verdict first. If there are findings, lead with them; otherwise give the ACCEPT rationale. Include the decisive evidence, meaningful review aspects, residual risks, and quest documentation hygiene judgment. Do not restate the entire diff or every green command.
