  ...
  useEffect(() => {
    ...
    store.applySyncedProjectionSnapshot({
      ...
      value: {
        ...
        tabState: { version: 1 },
        tabs: [
          ["q-1944", "Completed tab must stay passive"],
          ["q-1945", "Viewed tab keeps its position"],
          ["q-1946", "Later retained tab"],
        ].map(([threadKey, title]) => ({
          threadKey,
          questId: threadKey,
          title,
    ...
    seedCodexTurnRecoveryPlaygroundStates(store, session);
