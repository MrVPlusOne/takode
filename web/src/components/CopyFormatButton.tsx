...
export function CopyFormatButton({
  ...
  const handleCopyMarkdown = useCallback(() => {
    writeClipboardText(markdownText).then(() => showFeedback("Markdown")).catch(console.error);
  }, [markdownText, showFeedback]);

  const handleCopyPlainText = useCallback(() => {
    writeClipboardText(plainText).then(() => showFeedback("Plain text")).catch(console.error);
  }, [plainText, showFeedback]);

  const handleCopyRichText = useCallback(() => {
    const html = getHtml?.() ?? "";
    copyRichText(html, plainText).then(() => showFeedback("Rich text")).catch(console.error);
