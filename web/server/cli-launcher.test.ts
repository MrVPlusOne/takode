...
describe("launch", () => {
  ...
  it("strips inherited OTEL env vars from host CLI sessions", async () => {
  ...
  it("uses an explicit env-key model provider when no session auth.json is available", async () => {
    mockResolveBinary.mockReturnValue("/opt/fake/codex");
    mockSpawn.mockReturnValueOnce(createMockCodexProc());
  ...
  it("prefers session auth.json over OPENAI_API_KEY when both are present", async () => {
    mockResolveBinary.mockReturnValue("/opt/fake/codex");
    mockSpawn.mockReturnValueOnce(createMockCodexProc());
