...
- After alignment approval, authorize Work and Memory inside the approved envelope. Work is intentionally broader: provide only deltas the worker cannot infer from the Work brief, quest record, current artifacts, or its own context. The worker may self-review, run approved operations, sync/push when authorized, iterate, maintain the Work note, and use the worker-owned Work -> Memory transition once guard conditions are satisfied.
