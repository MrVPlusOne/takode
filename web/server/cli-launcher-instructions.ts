...
- Base branch: \`${syncBaseBranch}\``);
  ...
  parts.push(
    "## User notifications\n\n" +
      ...
      "For leader/orchestrator sessions, mark non-blocking thread status directly in assistant text with strict standalone status marker lines. Use `{[(Thread Waiting: main | waiting for herd event)]}` or `{[(Thread Ready: q-1258 | code review dispatched)]}`. The marker must occupy its own physical line, the target must be `main` or `q-N`, and the summary should be short plain text. These lines are stripped from displayed prose and rendered as thread-status chips. Multiple marker lines may appear in one response. Use `Thread Waiting` for non-user waits such as herd events, timers, resource leases, workers, reviewers, or queued dependencies. Use `Thread Ready` when the thread has a completed answer, accepted handoff, or ready-for-review result. Do not create `Thread Needs Input`; user-blocking prompts must use `takode notify needs-input`.\n\n" +
...
Use \`/quest-design\` before creating or materially refining quest text. As part of that approval, explicitly check whether the quest is a true follow-up to earlier work; if so, state \`Relationship: follow-up of [q-N](quest:q-N)\` and persist it with \`--follow-up-of\` after confirmation. Use \`/leader-dispatch\` before dispatching a fresh or newly refined quest so the user can approve the planned initial Journey before any worker is sent. In the common create-and-dispatch case, describe the proposed quest draft and the proposed Journey/scheduling plan together in prose, with one \`Goal / Scope\` that also serves as your understanding instead of duplicating the same requested work in multiple sections. If clarification is needed, ask it with quest framing; after the user clarifies and no major ambiguity remains, the next response should include both drafts rather than another restated-understanding-only round.
...
**Initial pre-dispatch approval is a combined contract.** Before you send the first worker message, get approval on both the initial Journey phases and the scheduling/orchestration plan, then write that approved Journey to the board before or with dispatch. Use one \`Goal / Scope\` section as both understanding and quest scope; add separate sections only for non-overlapping approval details such as \`Relationship\`, \`Evidence / Context\`, \`Boundaries / Non-goals\`, optional \`Assumptions / Open decisions\`, \`Invariants / Must preserve\`, \`Expected output / Acceptance\`, \`Journey\`, phase notes, and \`Scheduling\`. Always surface the expected worker choice or fresh-spawn intent, whether the quest will dispatch immediately or remain \`QUEUED\`, the exact \`--wait-for\` reason if queued, and whether you will archive a reclaimable completed worker before dispatching when capacity is tight. Even the simple case must stay explicit: "spawn fresh and dispatch immediately if approved." Omit notes for standard phases by default: \`alignment\`, \`implement\`, \`code-review\`, and \`port\` only need notes for unusual phase-specific work. Explain non-standard phases concisely: why the phase is needed and what evidence, user decision, scenario, outcome, or durable state it covers.
...
When a leader/orchestrator thread is not blocked on the user, mark its status with a strict standalone inline marker instead of a notify tool call: \`{[(Thread Waiting: main | waiting for herd event)]}\` or \`{[(Thread Ready: q-1258 | code review dispatched)]}\`. Use \`Thread Waiting\` for non-user waits and \`Thread Ready\` when the thread has a completed answer, accepted handoff, or ready-for-review result. The target must be \`main\` or \`q-N\`; the summary should be short plain text. These marker lines are stripped from displayed prose and rendered as thread-status chips. Do not create \`Thread Needs Input\`; user-blocking prompts must use \`takode notify needs-input\`.
After the user answers a same-session \`takode notify needs-input\` prompt, inspect your unresolved self-owned needs-input notifications with \`takode notify list\` and resolve the matching one with \`takode notify resolve <notification-id>\`. Use this only for notifications created by your current session, not herd notifications or other sessions.
