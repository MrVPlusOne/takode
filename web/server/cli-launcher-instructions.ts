...
- Base branch: \`${syncBaseBranch}\``);
  ...
  parts.push(
    "## User notifications\n\n" +
      ...
      "When a leader/orchestrator session needs input from the user, first publish the detailed question, decision options, or confirmation text with `takode user-message --text-file -`; normal worker and reviewer sessions should ignore `takode user-message` and use ordinary assistant text. After that text is visible, call `takode notify needs-input` with a short summary. Do not fire the notification before the detailed text is visible. When the answer choices are obvious and short, you may add one to three `--suggest <answer>` options such as `--suggest yes --suggest no`; never use suggestions instead of writing the full context in chat.\n\n" +
...
User-visible leader messages must be explicit: publish Markdown to the left panel with \`takode user-message --text-file -\`. Normal assistant output is leader activity, not the durable user-facing route. Worker and reviewer sessions should ignore \`takode user-message\`.
When a user decision is required, publish the detailed question, options, or confirmation text with \`takode user-message --text-file -\` first. After that user-visible text exists, call \`takode notify needs-input\` with a short summary. Do not fire the notification before the detailed text is visible. For obvious short choices, add one to three \`--suggest <answer>\` options; suggestions are only a reply convenience and never replace the detailed text.
After the user answers a same-session \`takode notify needs-input\` prompt, inspect your unresolved self-owned needs-input notifications with \`takode notify list\` and resolve the matching one with \`takode notify resolve <notification-id>\`. Use this only for notifications created by your current session, not herd notifications or other sessions.
Do not rely on deprecated leader reply suffixes like \`@to(user)\` or \`@to(self)\`. If repo-local docs still mention them, treat that guidance as stale and use \`takode user-message --text-file -\` plus \`takode notify\` when notification state is needed.
...
- **Never use \`AskUserQuestion\` or \`EnterPlanMode\`.** These block your turn and prevent you from processing herd events. Ask clarifying questions by publishing the detailed question or decision options with \`takode user-message --text-file -\`; after that text is visible, call \`takode notify needs-input\` with a short summary so the user never misses it. If the answer choices are obvious and short, include one to three \`--suggest <answer>\` flags. If you need a decision before dispatching, publish the options and wait for the user's next message.
...
- **Unresolved ambiguity blocks quest advancement.** If a worker/reviewer question exposes ambiguity you cannot resolve from existing context, publish the question with \`takode user-message --text-file -\`, then call \`takode notify needs-input\`, optionally with short \`--suggest\` choices for obvious answers, and stop advancing that quest until the ambiguity is resolved.
...
- **Rely on the right-side quest/status panel for rendered quest facts.** For the selected session or leader board attention row, the panel shows quest id/title/status, owner/session pointer, verification progress, inbox unread state, human feedback counts, compact wait/attention state, and existing compact Journey state. Do not repeat those facts in prose unless they are part of a concrete question. Keep reasoning, tradeoffs, decision prompts, worker/reviewer reports, and non-modeled operational facts in chat.
- **Use \`takode notify\` at these moments:**
  - \`needs-input\`: Every time you ask the user a question or need a user decision before work can continue. First publish the detailed question or decision text with \`takode user-message --text-file -\`, then call \`takode notify needs-input\` with a short summary so the user never misses it. Use \`--suggest\` only for concise obvious options, typically binary choices like yes/no.
