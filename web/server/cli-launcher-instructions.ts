...
- Base branch: \`${syncBaseBranch}\``);
  ...
  parts.push(
    "## User notifications\n\n" +
      ...
      "When you need input, first output the detailed question, decision options, or confirmation text in normal assistant text. After that text is complete, call `takode notify needs-input` with a short summary. Do not fire the notification before the detailed text is visible. When the answer choices are obvious and short, you may add one to three `--suggest <answer>` options such as `--suggest yes --suggest no`; never use suggestions instead of writing the full context in chat.\n\n" +
...
When a user decision is required, write the detailed question, options, or confirmation text in normal assistant text first. After that text is complete, call \`takode notify needs-input\` with a short summary. Do not fire the notification before the detailed text is visible. For obvious short choices, add one to three \`--suggest <answer>\` options; suggestions are only a reply convenience and never replace the detailed text.
After the user answers a same-session \`takode notify needs-input\` prompt, inspect your unresolved self-owned needs-input notifications with \`takode notify list\` and resolve the matching one with \`takode notify resolve <notification-id>\`. Use this only for notifications created by your current session, not herd notifications or other sessions.
...
- **Unresolved ambiguity blocks quest advancement.** If a worker/reviewer question exposes ambiguity you cannot resolve from existing context, ask the user in plain text first, then call \`takode notify needs-input\`, optionally with short \`--suggest\` choices for obvious answers, and stop advancing that quest until the ambiguity is resolved.
...
- **Use \`takode notify\` at these moments:**
  - \`needs-input\`: Every time you ask the user a question or need a user decision before work can continue. First output the detailed question or decision text, then call \`takode notify needs-input\` with a short summary so the user never misses it. Use \`--suggest\` only for concise obvious options, typically binary choices like yes/no.
