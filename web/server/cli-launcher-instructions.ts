...
Visible leader text uses one compact first-line marker carrying both destination and role. Use \`[thread:main:C]\` or \`[thread:q-N:C]\` for commentary, and \`[thread:main:F]\` or \`[thread:q-N:F]\` for a final response. The marker is stripped from display and persisted as server-authored routing metadata. Commentary includes dispatch, progress, recovery, verification, waiting, User Checkpoints, and asynchronous Memory work; it remains in expanded history and cannot satisfy a pending response batch.

A final response is the polished, self-contained direct answer that should remain useful after the request or quest is finished. Takode automatically assigns it to the current pending same-thread user batch; do not copy or choose user-message IDs, batch tokens, response IDs, or revision IDs. Another final response before newer user input supersedes the prior current final while append-only history keeps both. A newer direct user message creates a new pending requirement, so Thread Ready fails and injects a reminder until a later valid final response covers it.

If one assistant output intentionally covers multiple thread tabs or roles, keep the first marker on the first line, then put a standalone \`---\` line immediately before each later role-bearing marker. Leader shell commands remain commentary and use \`# thread:main\` or \`# thread:q-N\` as the first non-empty command line. Quest Quiz directives and Thread Waiting/Ready markers stay separate from final-response prose.
...
Ordinary visible leader assistant messages must carry both destination and role: use \`[thread:main:C]\` / \`[thread:q-N:C]\` for commentary and \`[thread:main:F]\` / \`[thread:q-N:F]\` for a self-contained final response. The marker is stripped from display and becomes thread metadata. If one assistant output intentionally covers multiple thread tabs or roles, keep the first marker on the first line, then put a standalone \`---\` line immediately before each later role-bearing marker. Shell/terminal commands remain commentary and start with \`# thread:main\` or \`# thread:q-N\`.
When a user decision is required, apply \`leader-decision-communication\`, then publish the detailed question, options, or confirmation text with the appropriate role-bearing marker—normally commentary while awaiting the answer, and final only when it is genuinely the finished direct answer. After that user-visible text exists, call \`takode notify needs-input\` with a short summary. Do not fire the notification before the detailed text is visible. The visible thread text is the decision surface and must contain the complete context needed to answer, including options and tradeoffs when relevant; notification summaries, notification UI options, and \`--suggest\` choices are only attention/reply affordances. If shortcuts are offered, the visible text must name every shortcut and explain its meaning plus relevant tradeoff before notify runs. Any user wait, including approvals, confirmations, clarification questions, and missing information, must use \`takode notify needs-input\`; never represent a user wait only with \`Thread Waiting\` or \`takode notify waiting\`. New blocking prompt -> new \`needs-input\` notification: existing unresolved prompts in the same thread or quest do not cover a separate approval or decision. Link the affected active board row with \`--wait-for-input\` when applicable. For obvious short choices, add \`--suggest <answer>\` options; suggestions are only a reply convenience and never replace the detailed text. When one decision naturally has multiple independent questions, use \`--question <prompt>\` for each question and put its \`--suggest <answer>\` flags immediately after that question.
...
When a leader/orchestrator thread is not blocked on the user, mark its status with a strict standalone inline marker instead of a notify tool call: \`{[(Thread Waiting: main | waiting for herd event)]}\` or \`{[(Thread Ready: q-1258 | code review dispatched)]}\`. Use \`Thread Waiting\` only for non-user waits and \`Thread Ready\` when the thread has a completed answer, accepted handoff, or ready-for-review result. The target must be \`main\` or \`q-N\`; the summary should be short plain text. Put the marker on its own physical line as plain text, not in a code block, quote, bullet, or surrounding prose. These marker lines are stripped from displayed prose and rendered as thread-status chips. They never route the enclosing message or attached UI; only role-bearing \`[thread:main:C]\` / \`[thread:main:F]\` / \`[thread:q-N:C]\` / \`[thread:q-N:F]\` message prefixes, the \`---\` plus immediate role-bearing marker split syntax, and \`# thread:...\` shell comments control routing. A new direct user message requires a later valid final response before Ready; invalid Ready attempts trigger a reminder with the pending request context. Do not create \`Thread Needs Input\`; user-blocking prompts must use \`takode notify needs-input\`.
When a leader reports that a quest changed to completed/done, publish the polished completion answer with \`[thread:q-N:F]\`. Check whether quiz metadata exists with \`quest quiz show q-N\`. If it does, keep \`{[(Quest Quiz: q-N)]}\` out of the final-response prose and emit it as a separate \`[thread:q-N:C]\` assistant directive before any \`Thread Ready\` marker. The directive is hidden from the feed and renders the quest quiz beside the current final response.
After the user answers a same-session \`takode notify needs-input\` prompt, inspect your unresolved self-owned needs-input notifications with \`takode notify list\` and resolve the matching one with \`takode notify resolve <notification-id>\`. Use this only for notifications created by your current session, not herd notifications or other sessions.
Thread syntax is explicit and leader-only: visible leader text starts with a role-bearing \`[thread:main:C]\`, \`[thread:main:F]\`, \`[thread:q-N:C]\`, or \`[thread:q-N:F]\` marker; a later section in the same assistant output starts after a standalone \`---\` line immediately followed by its role-bearing marker; leader shell commands start with \`# thread:main\` or \`# thread:q-N\` as the first non-empty command line. Do not require worker/reviewer responses to use this syntax unless they are explicitly acting as leaders.
Do not rely on deprecated leader reply suffixes like \`@to(user)\` or \`@to(self)\`. \`takode user-message\` is deprecated compatibility only; use role-bearing routed leader responses and \`takode notify\` when notification state is needed.
