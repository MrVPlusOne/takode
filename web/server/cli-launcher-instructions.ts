...
- Base branch / port target: \`${syncBaseBranch}\`${portTargetWorktree}${portTargetSource}`);
  ...
  parts.push(
    "## User notifications\n\n" +
      ...
      "When a leader reports that a quest changed to completed/done, publish the polished completion response with `takode thread-response set`. Check whether quiz metadata exists with `quest quiz show q-N`. If it does, keep `{[(Quest Quiz: q-N)]}` out of the response Markdown and emit it as a separate ordinary routed assistant directive before any Thread Ready marker. The directive is hidden from the feed and renders the quest quiz beside the current response.\n\n" +
...
## Leader Thread Responses

Use \`takode thread-response show --thread <main|q-N>\` to inspect the server-owned pending user-request batches and current responses for one leader thread. Normal output shows counts and previews without requiring user-message IDs; add \`--history\` only for detailed audit/revision evidence.

Use \`takode thread-response set --thread <main|q-N> --text-file <path|->\` for the polished response to the current request batch. The CLI carries the opaque server-owned batch identity internally: if a batch is pending, it creates one logical response for the oldest pending batch; if none is pending, it revises the latest logical response. New user input that arrives concurrently is not silently absorbed into the observed batch and remains pending for a later response. Do not copy message IDs, response IDs, batch tokens, or revision IDs into normal authoring commands.

The command itself publishes the visible routed response, so do not repeat the same prose in a normal assistant response. Temporary progress may remain ordinary \`[thread:...]\` activity, but it is not the polished final response. The explicit \`--thread\` is authoritative; never infer destination from the active browser tab, a quest claim, prose, or a preceding \`# thread:...\` shell marker. Keep Quest Quiz, Thread Waiting/Ready, and needs-input notification state separate. If the response asks a blocking question, publish the complete decision surface first and then create the required same-thread \`takode notify needs-input\`.
...
Ordinary user-visible leader assistant messages must be explicitly routed: every ordinary response starts with \`[thread:main]\` or \`[thread:q-N]\`. A \`takode thread-response set\` publication is routed instead by its required \`--thread\` flag and does not embed the marker in its Markdown. The ordinary-response marker is stripped from display and becomes thread metadata. If one leader response intentionally covers multiple thread tabs, keep the first-line marker for the first tab, then put a standalone \`---\` line immediately before each later \`[thread:main]\` or \`[thread:q-N]\` marker. Shell/terminal commands that belong to a thread should start with \`# thread:main\` or \`# thread:q-N\`.
When a user decision is required, apply \`leader-decision-communication\`, then publish the detailed question, options, or confirmation text with \`takode thread-response set\` when it is the polished response to the current pending batch, or as an ordinary marked leader response otherwise. Do not publish both copies. After that user-visible text exists, call \`takode notify needs-input\` with a short summary. Do not fire the notification before the detailed text is visible. The visible thread text is the decision surface and must contain the complete context needed to answer, including options and tradeoffs when relevant; notification summaries, notification UI options, and \`--suggest\` choices are only attention/reply affordances. If shortcuts are offered, the visible text must name every shortcut and explain its meaning plus relevant tradeoff before notify runs. Any user wait, including approvals, confirmations, clarification questions, and missing information, must use \`takode notify needs-input\`; never represent a user wait only with \`Thread Waiting\` or \`takode notify waiting\`. New blocking prompt -> new \`needs-input\` notification: existing unresolved prompts in the same thread or quest do not cover a separate approval or decision. Link the affected active board row with \`--wait-for-input\` when applicable. For obvious short choices, add \`--suggest <answer>\` options; suggestions are only a reply convenience and never replace the detailed text. When one decision naturally has multiple independent questions, use \`--question <prompt>\` for each question and put its \`--suggest <answer>\` flags immediately after that question.
...
When a leader reports that a quest changed to completed/done, publish the polished completion response with \`takode thread-response set\`. Check whether quiz metadata exists with \`quest quiz show q-N\`. If it does, keep \`{[(Quest Quiz: q-N)]}\` out of the response Markdown and emit it as a separate ordinary routed assistant directive before any \`Thread Ready\` marker. The directive is hidden from the feed and renders the quest quiz beside the current response.
After the user answers a same-session \`takode notify needs-input\` prompt, inspect your unresolved self-owned needs-input notifications with \`takode notify list\` and resolve the matching one with \`takode notify resolve <notification-id>\`. Use this only for notifications created by your current session, not herd notifications or other sessions.
Thread syntax is explicit and leader-only: ordinary visible leader messages start with \`[thread:main]\` or \`[thread:q-N]\`, while \`takode thread-response set\` uses its required \`--thread\` flag; a later thread tab in the same visible leader message starts after a standalone \`---\` line immediately followed by \`[thread:main]\` or \`[thread:q-N]\`; leader shell commands start with \`# thread:main\` or \`# thread:q-N\` as the first non-empty command line. Do not require worker/reviewer responses to use this syntax unless they are explicitly acting as leaders.
Do not rely on deprecated leader reply suffixes like \`@to(user)\` or \`@to(self)\`. \`takode user-message\` is deprecated compatibility only; use marked leader responses for temporary activity, \`takode thread-response set\` for polished request responses, and \`takode notify\` when notification state is needed.
