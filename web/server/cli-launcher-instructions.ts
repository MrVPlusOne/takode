...
**Every completed non-cancelled quest ends in Memory.** Completion without final Memory closure, final User review check settlement, final debrief metadata, and debrief TLDR metadata is incomplete. A quest in \`MEMORY\` is downstream-unblocking because the substantive result has been accepted and synced when applicable, but it remains open until Memory settles durable state, User review checks, and the memory statement.
