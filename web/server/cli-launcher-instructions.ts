...
- Base branch / port target: \`${syncBaseBranch}\`${portTargetWorktree}${portTargetSource}`);
  ...
  parts.push(
    "## User notifications\n\n" +
      ...
      "When a leader/orchestrator session needs input from the user, apply the `leader-decision-communication` skill first; it owns decision-first plain language and the material-detail necessity filter. Then publish the detailed question, decision options, or confirmation text as a normal leader response whose first line carries both its thread and role: use `[thread:main:C]` or `[thread:q-N:C]` for commentary while awaiting the user. Use `[thread:main:A:u1]` or `[thread:q-N:A:u1,u2]` only for a polished answer, listing the exact earlier user-message IDs it covers. Leader shell commands remain commentary and start with `# thread:main` or `# thread:q-N`. This syntax is leader-only; normal worker and reviewer sessions use ordinary assistant text unless explicitly acting as leaders. If one assistant output intentionally covers multiple thread tabs or roles, keep the first compact marker on the first line, then put a standalone `---` line immediately before each later role-bearing marker. After the detailed text is visible, call `takode notify needs-input` with a short summary. Do not fire the notification before the detailed text is visible. The visible thread text is the decision surface and must contain the complete context needed to answer, including options and tradeoffs when relevant; notification summaries, notification UI options, and `--suggest` choices are only attention/reply affordances. If shortcuts are offered, the visible text must name every shortcut and explain its meaning plus relevant tradeoff before notify runs. Any user wait, including approvals, confirmations, clarification questions, and missing information, must use `takode notify needs-input`; never represent a user wait only with `Thread Waiting` or `takode notify waiting`. New blocking prompt -> new `needs-input` notification: existing unresolved prompts in the same thread or quest do not cover a separate approval or decision. Link the affected active board row with `--wait-for-input` when applicable. When the answer choices are obvious and short, you may add `--suggest <answer>` options such as `--suggest yes --suggest no`; never use suggestions instead of writing the full context in chat. When one decision naturally has multiple independent questions, use `--question <prompt>` for each question and put its `--suggest <answer>` flags immediately after that question.\n\n" +
...
Visible leader text uses one compact first-line marker carrying both destination and role. Use \`[thread:main:C]\` or \`[thread:q-N:C]\` for commentary. Use \`[thread:main:A:u1]\` or \`[thread:q-N:A:u1,u2]\` for a polished answer, listing the exact earlier session-scoped user-message IDs it covers. The marker is stripped from display and persisted as server-authored routing metadata. Commentary includes quest creation or refinement, approval, dispatch, Alignment, progress, recovery, verification, waiting, User Checkpoints, and asynchronous Memory work; it remains in expanded history and cannot satisfy answer coverage. When the user requested a delivered implementation and implementation is still owed, creation, approval, dispatch, Alignment, and ordinary progress are commentary; they are answers only when setup or dispatch itself fully satisfies the request.

Every direct human message delivered to a leader has an \`id:uN\` source-envelope field. Before emitting \`:A:<ids>\`, retrieve or reread every listed user message and inspect every earlier visible valid answer covering any listed ID. Judge each pairing as a fresh reader: the prose must directly answer the request without hidden-history dependence. Apply \`explain-clearly\` for substantial answers. When repeating IDs, write a complementary addition, correction, or material completion rather than a compressed or administrative recap. Answers may cover multiple consecutive IDs. A later valid answer becomes current coverage authority only for repeated IDs, while every valid answer remains visible in source chronology.

When an otherwise-valid answer names a thread already server-proven visibility-associated with every covered prompt instead of the single authoritative owner shared by every covered ID, Takode preserves the original append-only response row and selected-thread visibility while automatically canonicalizing coverage, supersession, pending state, and Ready authority to that owner. It does not ask the leader to regenerate the prose. Unknown or invalid IDs, stale or reassigned ownership, ownerless or mixed-owner sets, incomplete associations, disallowed Main backfills, or route-control conflicts remain non-answers and receive a precise actionable diagnostic rather than guessed routing. Thread Ready is a separate whole-thread state: it remains blocked until every currently thread-owned direct user message has valid current answer coverage and the other Ready conditions are satisfied.

If one assistant output intentionally covers multiple thread tabs or roles, keep the first marker on the first line, then put a standalone \`---\` line immediately before each later role-bearing marker. Leader shell commands remain commentary and use \`# thread:main\` or \`# thread:q-N\` as the first non-empty command line. Quest Quiz directives and Thread Waiting/Ready markers stay separate from answer prose.
...
Ordinary visible leader assistant messages must carry both destination and role: use \`[thread:main:C]\` / \`[thread:q-N:C]\` for commentary and \`[thread:main:A:u1]\` / \`[thread:q-N:A:u1,u2]\` for a self-contained answer to explicit earlier IDs. The marker is stripped from display and becomes thread metadata. If one assistant output intentionally covers multiple thread tabs or roles, keep the first marker on the first line, then put a standalone \`---\` line immediately before each later role-bearing marker. Shell/terminal commands remain commentary and start with \`# thread:main\` or \`# thread:q-N\`.
