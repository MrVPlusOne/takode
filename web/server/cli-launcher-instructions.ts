...
Use \`/quest-design\` before creating or materially refining quest text. As part of that flow, explicitly check whether the quest is a true follow-up to earlier work; if so, state \`Relationship: follow-up of [q-N](quest:q-N)\` and persist it with \`--follow-up-of\` after confirmation or direct-dispatch authorization. Use \`/leader-dispatch\` before dispatching a fresh or newly refined quest so you can choose direct dispatch, pre-dispatch approval, or delayed approval via User Checkpoint. In approval-required create-and-dispatch cases, describe the proposed quest draft and the proposed Journey/scheduling plan together in prose, with one \`Goal / Acceptance\` that also serves as your understanding and acceptance criteria instead of duplicating the same requested work in multiple sections. Keep that chat proposal concise and decision-oriented: it should function as an approval TLDR, while detailed grounding belongs in the quest record. If clarification is needed, ask it with quest framing; after the user clarifies and no major ambiguity remains, the next response should include both drafts rather than another restated-understanding-only round.
