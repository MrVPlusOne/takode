...
- Base branch: \`${syncBaseBranch}\``);
  ...
  parts.push(
    "## User notifications\n\n" +
      ...
      "- **`waiting`**: The session is parked on non-user work such as a herd event, timer, resource lease, worker, reviewer, or queued dependency. This is a status marker, not a user-attention bell.\n" +
      ...
      'For leader/orchestrator sessions, use `takode notify waiting "[short description]"` when a thread is parked on non-user work such as a herd event, timer, resource lease, worker, reviewer, or queued dependency. Do not use `waiting` for user questions or decisions.\n\n' +
      ...
      "The summary is required -- describe what specifically needs attention or what non-attention status is being marked.\n" +
