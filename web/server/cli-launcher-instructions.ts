...
- Base branch: \`${syncBaseBranch}\``);
  ...
  parts.push(
    "## User notifications\n\n" +
      ...
      "- **`waiting`**: Legacy CLI status for sessions that are parked on non-user work. Leader/orchestrator threads should prefer inline `Thread Waiting` markers in assistant text instead.\n" +
...
Use \`/quest-design\` before creating or materially refining quest text. As part of that approval, explicitly check whether the quest is a true follow-up to earlier work; if so, state \`Relationship: follow-up of [q-N](quest:q-N)\` and persist it with \`--follow-up-of\` after confirmation. Use \`/leader-dispatch\` before dispatching a fresh or newly refined quest so the user can approve the planned initial Journey before any worker is sent. In the common create-and-dispatch case, describe the proposed quest draft and the proposed Journey/scheduling plan together in prose, with one \`Goal / Acceptance\` that also serves as your understanding and acceptance criteria instead of duplicating the same requested work in multiple sections. If clarification is needed, ask it with quest framing; after the user clarifies and no major ambiguity remains, the next response should include both drafts rather than another restated-understanding-only round.
...
**Zero-tracked-change quests still use explicit Journey phases.** If the accepted result truly produced zero git-tracked changes, model that by choosing a phase plan that omits \`port\` but still ends in \`memory\`; do not use a separate board-side no-code path. Finish those quests without \`/port-changes\`, synced SHA placeholders, or fake port-summary comments. Docs, skills, prompts, templates, and other text-only tracked-file edits are commit-producing work: port them normally and attach their synced SHAs before final Memory. If you use \`quest complete ... --no-code\`, treat it only as a local CLI reminder switch, not durable quest metadata.
**Every completed non-cancelled quest ends in Memory.** Completion without final Memory closure, final debrief metadata, and debrief TLDR metadata is incomplete. A quest in \`MEMORY\` is downstream-unblocking because the substantive result has been accepted and synced when applicable, but it remains open until Memory settles durable state and the memory statement.
...
**Bookkeeping is compatibility-only for targeted intermediate durable state.** Use \`bookkeeping\` for cross-phase or external durable state beyond normal phase notes only when it is not final Memory closure: consolidated summaries, verification checklist reconciliation, external docs or links, superseded facts, notification cleanup, thread cleanup, or shared-state updates. Do not dispatch Bookkeeping as a substitute for mandatory final Memory.
...
**Initial pre-dispatch approval is a combined contract.** Before you send the first worker message, get approval on both the initial Journey phases and the scheduling/orchestration plan, then write that approved Journey to the board before or with dispatch. Use one \`Goal / Acceptance\` section as both understanding and acceptance criteria; add separate sections only for non-overlapping approval details such as \`Relationship\`, \`Context / Evidence\`, optional \`Out Of Scope\`, optional \`Open Questions\`, \`Invariants / Must Preserve\`, \`Journey\`, phase notes, and \`Scheduling\`. Always surface the expected worker choice or fresh-spawn intent, whether the quest will dispatch immediately or remain \`QUEUED\`, the exact \`--wait-for\` reason if queued, and whether you will archive a reclaimable completed worker before dispatching when capacity is tight. Even the simple case must stay explicit: "spawn fresh and dispatch immediately if approved." Omit notes for standard phases by default: \`alignment\`, \`implement\`, \`code-review\`, \`port\`, and \`memory\` only need notes for unusual phase-specific work. Explain non-standard phases concisely: why the phase is needed and what evidence, user decision, scenario, outcome, or durable state it covers.
...
When a leader/orchestrator thread is not blocked on the user, mark its status with a strict standalone inline marker instead of a notify tool call: \`{[(Thread Waiting: main | waiting for herd event)]}\` or \`{[(Thread Ready: q-1258 | code review dispatched)]}\`. Use \`Thread Waiting\` for non-user waits and \`Thread Ready\` when the thread has a completed answer, accepted handoff, or ready-for-review result. The target must be \`main\` or \`q-N\`; the summary should be short plain text. Put the marker on its own physical line as plain text, not in a code block, quote, bullet, or surrounding prose. These marker lines are stripped from displayed prose and rendered as thread-status chips. They never route the enclosing message or attached UI; only \`[thread:main]\` / \`[thread:q-N]\` message prefixes and \`# thread:...\` shell comments control routing. Do not create \`Thread Needs Input\`; user-blocking prompts must use \`takode notify needs-input\`.
After the user answers a same-session \`takode notify needs-input\` prompt, inspect your unresolved self-owned needs-input notifications with \`takode notify list\` and resolve the matching one with \`takode notify resolve <notification-id>\`. Use this only for notifications created by your current session, not herd notifications or other sessions.
...
- **If you asked the user a question, wait only on the affected scope.** Do not advance the thread, quest, or board row covered by that pending decision until the user responds. Process herd events and continue unrelated quests normally. Treat the wait as global only when your visible question explicitly says it concerns global orchestration, worker-slot scheduling, shared resource safety, or another cross-quest dependency.
...
- **Use \`takode notify\` at these moments:**
  ...
  - \`waiting\`: Legacy CLI fallback for non-user waits; prefer inline \`Thread Waiting\` markers in leader responses so the status is visible without an extra tool call.
