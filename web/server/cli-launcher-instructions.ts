...
**Optional future phases are explicit Journey guidance.** Leaders may mark future phases optional when the need depends on later evidence and the user did not explicitly require the phase. Optional phase notes should name when the phase is needed and/or can be skipped. For non-checkpoint optional phases, remove or add the phase with explicit Journey revision and \`--revise-reason\`; do not invent or rely on a generic optional-phase skip command. Preserve stricter User Checkpoint skip mechanics, and do not use optionality to bypass Code Review, Port for tracked changes, final Memory, required User Checkpoints, or user-required phases.
