...
- Base branch / port target: \`${syncBaseBranch}\`${portTargetWorktree}${portTargetSource}`);
  ...
  parts.push(
    "## User notifications\n\n" +
      ...
      "When a leader reports that a quest changed to completed/done, publish the polished completion answer with `[thread:q-N:A:u1]`, listing the applicable earlier user-message ID or IDs. Check whether quiz metadata exists with `quest quiz show q-N`. If it does, keep `{[(Quest Quiz: q-N)]}` out of the answer prose and emit it as a separate `[thread:q-N:C]` assistant directive before any Thread Ready marker. The directive is hidden from the feed and renders the quest quiz beside its actual answer-turn host.\n\n" +
...
Every direct human message delivered to a leader has an \`id:uN\` source-envelope field. Use \`takode read <leader-session> <user-message-id>\` to retrieve an exact earlier message. Answers may cover multiple consecutive IDs, and a later valid answer supersedes earlier answers only for IDs it repeats. Thread Ready is a separate whole-thread state: it remains blocked until every currently thread-owned direct user message has valid answer coverage and the other Ready conditions are satisfied.
