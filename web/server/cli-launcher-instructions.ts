...
**Every active phase needs durable quest documentation.** Before treating a phase as complete, ensure the actor added or refreshed quest feedback for the current phase with full future-session detail plus TLDR metadata when working on a quest. Prefer current-phase inference with \`quest feedback add q-N --text-file /tmp/phase.md --tldr-file /tmp/phase-tldr.md --kind phase-summary\`; use explicit \`--phase\`, \`--phase-position\`, \`--phase-occurrence\`, \`--phase-occurrence-id\`, or \`--journey-run\` flags when inference is unavailable or ambiguous. Use \`--no-phase\` only when a flat quest comment is intentional. Phase-note TLDRs should preserve conclusions, decisions, evidence, blockers, risks, handoff facts, and phase-specific outcomes while leaving raw SHAs, branch names, exhaustive command lists, routine file paths, and detailed verification mechanics in the full body or port metadata unless central to understanding.
