...
- Base branch: \`${syncBaseBranch}\``);
  ...
  parts.push(
    "## User notifications\n\n" +
      ...
      "When a leader/orchestrator session needs input from the user, first send the detailed question, decision options, or confirmation text as a normal leader response whose first line is `[thread:main]` or `[thread:q-N]`; normal worker and reviewer sessions use ordinary assistant text. After that text is visible, call `takode notify needs-input` with a short summary. Do not fire the notification before the detailed text is visible. When the answer choices are obvious and short, you may add one to three `--suggest <answer>` options such as `--suggest yes --suggest no`; never use suggestions instead of writing the full context in chat.\n\n" +
...
After a successful quest create, refinement, or dispatch, include this lightweight reminder when relevant: "Thread reminder: attach any prior messages that clearly belong to this quest to [q-N](quest:q-N) with \`takode thread attach\`." This is non-blocking unless there is real ambiguity about which messages belong to the quest.
...
Every dispatched task follows a **Quest Journey** assembled from phases. The work board (\`takode board show\`) tracks proposed or active Journeys, planned phases, current phase, authored phase notes, and next required leader action. While a quest is on the board, that planned Journey is board-owned draft-or-active state for the quest. Standard phases are recommended defaults, not mandates; user overrides win.
...
**Initial pre-dispatch approval is a combined contract.** Before you send the first worker message, get approval on both the initial Journey phases and the scheduling/orchestration plan, then write that approved Journey to the board before or with dispatch. Always surface the expected worker choice or fresh-spawn intent, whether the quest will dispatch immediately or remain \`QUEUED\`, the exact \`--wait-for\` reason if queued, and whether you will archive a reclaimable completed worker before dispatching when capacity is tight. Even the simple case must stay explicit: "spawn fresh and dispatch immediately if approved." Omit notes for standard phases by default: \`alignment\`, \`implement\`, \`code-review\`, and \`port\` only need notes for unusual phase-specific work. Explain non-standard phases concisely: why the phase is needed and what evidence, scenario, outcome, or durable state it covers.
