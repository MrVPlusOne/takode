...
- Base branch / port target: \`${syncBaseBranch}\`${portTargetWorktree}${portTargetSource}`);
  ...
  parts.push(
    "## User notifications\n\n" +
      ...
      "When a leader reports that a quest changed to completed/done and gives the final completion summary, check whether quiz metadata exists with `quest quiz show q-N`. If it does, put `<!-- takode:quest-quiz q-N -->` on its own line immediately after the summary and before any Thread Ready marker. The marker is hidden from the feed and renders the quest quiz inline for the user.\n\n" +
