...
- Base branch: \`${syncBaseBranch}\``);
  ...
  parts.push(
    "## User notifications\n\n" +
      ...
      "- **`waiting`**: The session or scoped thread is parked on non-user work such as a lease, timer, worker, reviewer, or queued dependency. This is a transient status marker, not an unresolved user notification; it does not receive an ID, does not appear in notification chips, and is not resolved with `takode notify resolve`.\n" +
      ...
      'For leader/orchestrator sessions, use `takode notify waiting "[short description]"` when a thread is parked on non-user work such as a herd event, timer, resource lease, worker, reviewer, or queued dependency. Do not use `waiting` for user questions or decisions; it is status only, not resolver work.\n\n' +
      ...
      "The summary is required -- describe what specifically needs attention for `needs-input`/`review`, or what non-user work is being waited on for `waiting`.\n" +
...
When a leader/orchestrator thread is not blocked on the user but is genuinely waiting, call \`takode notify waiting "[short description]"\` as a non-attention status marker. Do not use \`waiting\` for user questions or decision prompts.
After the user answers a same-session \`takode notify needs-input\` prompt, inspect your unresolved self-owned needs-input notifications with \`takode notify list\` and resolve the matching one with \`takode notify resolve <notification-id>\`. Use this only for notifications created by your current session, not herd notifications or other sessions.
...
- **Use \`takode notify\` at these moments:**
  ...
  - \`waiting\`: When a marked leader thread is parked on a non-user dependency such as a herd event, timer, resource lease, worker, reviewer, or queued dependency. This keeps the thread status visible without creating an amber prompt.
  ...
  - \`waiting\`: Use this only when a thread, quest, or board row is intentionally parked on non-user work. It is transient status, not user attention: it does not create notification IDs, chip counts, or resolver work.
${copy.delegationLine}
