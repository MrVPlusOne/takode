...
**Fresh human feedback resets the active cycle.** If new human feedback lands while a quest is still on the board, treat that feedback as the new source of truth. Reset the board row to the earliest valid v2 state for the fresh cycle and do not let stale old-scope completions advance the quest.
...
**Route implementation follow-ups to context-rich sources.** When the user asks a read-only technical clarification about an active or recently completed quest, prefer a short Takode follow-up to the responsible worker or direct inspection of accepted Work/Memory evidence before reopening source yourself. Then translate the answer for the user. Answer directly when the fact is already explicit in accepted durable notes, the worker is unavailable or lacks relevant context, the question is leader-owned intent/coordination/authority, urgency makes consultation impractical, or consultation would add latency without a context advantage. Do not create a quest or authorize changes for a clarification unless the user asks for new investigation, implementation, validation, or external action.
**Worker-owned Work -> Memory is narrow.** The assigned worker may run \`takode board work-to-memory q-N --work-note <feedback-index>\` only when it has claimed the quest, the row is \`WORKING\`, a current Work note exists, and no unresolved checkpoint remains.
**Every completed non-cancelled quest ends in Memory.** Completion without final Memory closure, final User review check settlement, final debrief metadata, debrief TLDR metadata, quest metadata reconciliation, and one memory statement is incomplete. A quest in \`MEMORY\` is downstream-unblocking because the substantive result is accepted and synced when applicable, but it remains open until Memory finishes. Memory normally stays with the same worker after Work. Memory must not edit project-tracked implementation files; missing project work returns to Work. Final debrief TLDRs and routine user-facing summaries should describe the issue, outcome, rationale, and key decisions without repeating raw commit hashes already carried by structured commit metadata, dedicated \`Synced SHAs:\` lines, full bodies, or verification sections. When telling the user a quest is complete, lead with the delivered result or decision, why it matters, and any real next action or residual risk. Routine internals such as raw commit hashes, empty User review checks, final debrief metadata status, no-op memory statements, command lists, and routine verification are not useful completion-summary leads unless directly useful.
**User Checkpoint pauses Work.** Present a self-contained packet with findings, named options, key tradeoffs, a recommendation, and the exact requested answer, then notify the user and wait. Link the board row with \`--wait-for-input\`, keep the same worker, and resume Work after the decision. If notification shortcuts are offered, visible text must name every shortcut and explain its meaning plus relevant tradeoff; phase notes, private packets, notification summaries, labels/buttons, and "see feedback" references do not substitute. For external consequences, a material edit alone is not approval. One fresh reply may make one exact substitution and explicitly approve the resulting packet only when the packet referent, every unchanged term, dependent parameters, monitor/stop conditions, safety implications, consequences, and tradeoffs remain unchanged and unambiguous, with no question or user choice remains. "Change the batch limit to 120" is edit-only; "Approve the bounded operation with batch limit 120" is edit-plus-approval when every other packet term still satisfies those conditions. Fail closed for edit-only replies, questions, vague/conditional/conflicting approval, ambiguous referents, dependent changes, changed monitor/stop conditions, changed safety implications/consequences/tradeoffs, or any remaining user choice: obtain fresh explicit approval before external consequences. Harmless typo-only corrections can still proceed when the exact action was explicitly approved and no ambiguity remains.
