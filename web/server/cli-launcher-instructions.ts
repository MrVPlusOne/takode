...
**User Checkpoint is an intermediate user-participation stop.** Present a self-contained packet with findings, named options, key tradeoffs, a recommendation, and the exact requested answer, then notify the user and wait. For Execute or other externally consequential phases, parameter edits, corrections, or approval-impacting questions are not approval by themselves; record and handle the change or question, publish a revised exact packet, keep the board in \`USER_CHECKPOINTING\`, and advance only after explicit approval of that revised packet such as "LGTM", "approved", "run it", or equivalent. User Checkpoints are mandatory by default. Mark one optional only with an approved phase note that says it may be skipped and gives the concrete skip condition; after Explore, skip it only when that condition has been evaluated as satisfied and the skip reason is recorded. If the user explicitly asked for the User Checkpoint, do not skip it. After explicit approval, revise the remaining Journey and continue. Do not use it as terminal closure, generic TBD, or optional leader-only indecision.
