...
describe("resolveAudioUploadFormat", () => {
  it("normalizes browser codec suffixes for mp4 recordings", () => {
    ...
    expect(
      resolveAudioUploadFormat(buffer, "audio/mp4;codecs=mp4a.40.2", "recording.mp4"),
    ).toEqual({
      mimeType: "audio/mp4",
      extension: "mp4",
    });
  ...
  it("prefers sniffed mp4 bytes over a mislabeled webm mime type", () => {
    ...
    expect(
      resolveAudioUploadFormat(buffer, "audio/webm", "recording.webm"),
    ).toEqual({
      mimeType: "audio/mp4",
      extension: "mp4",
    });
  ...
  it("accepts video/webm recorder outputs for audio-only captures", () => {
    ...
    expect(
      resolveAudioUploadFormat(buffer, "video/webm", "recording.webm"),
    ).toEqual({
      mimeType: "audio/webm",
      extension: "webm",
    });
  ...
  it("falls back to the filename extension when mime type is missing", () => {
    ...
    expect(
      resolveAudioUploadFormat(buffer, "", "recording.ogg"),
    ).toEqual({
      mimeType: "audio/ogg",
      extension: "ogg",
    });
