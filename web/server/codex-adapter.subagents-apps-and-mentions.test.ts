...
describe("CodexAdapter", () => {
  ...
  it("renders Takode delegate_task MCP calls as Agent-shaped subagent cards", async () => {
    const messages: BrowserIncomingMessage[] = [];
    const adapter = new CodexAdapter(proc as never, "test-session", { model: "o4-mini" });
    adapter.onBrowserMessage((msg) => messages.push(msg));
      ...
      JSON.stringify({
        ...
        params: {
          ...
          item: {
            ...
            tool: "delegate_task",
            arguments: { task: "Search for large-output call sites under web and summarize the count." },
            status: "inProgress",
          },
        },
      }) + "\n",
    ...
    expect(block?.input).toMatchObject({
      ...
      task: "Search for large-output call sites under web and summarize the count.",
      prompt: "Search for large-output call sites under web and summarize the count.",
    });
  });

  it("preserves legacy delegate_command MCP rows as Agent-shaped subagent cards", async () => {
    const messages: BrowserIncomingMessage[] = [];
    const adapter = new CodexAdapter(proc as never, "test-session", { model: "o4-mini" });
    adapter.onBrowserMessage((msg) => messages.push(msg));
    ...
    const agentToolUse = messages.find((msg) => {
      if (msg.type !== "assistant") return false;
      return msg.message.content.some(
        (block) => block.type === "tool_use" && block.id === "delegate_call_legacy" && block.name === "Agent",
      );
    }) as
      | {
          message: { content: Array<{ type: string; id?: string; name?: string; input?: Record<string, unknown> }> };
    ...
    expect(agentToolUse).toBeDefined();
    const block = agentToolUse!.message.content.find(
      (item) => item.type === "tool_use" && item.id === "delegate_call_legacy",
    );
    expect(block?.input).toMatchObject({
