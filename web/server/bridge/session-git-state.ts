...
interface ComputeDiffStatsOptions {
  skipNonWorktreeDefaultMainDiffStats?: boolean;
}
...
interface RecomputeDiffIfDirtyDeps {
  ...
  isHeavyRepoModeEnabled?: () => boolean;
}
...
function shouldSkipNonWorktreeDefaultMainDiffStats(
  session: SessionDiffStateLike,
  diffBase: string,
  options: ComputeDiffStatsOptions,
): boolean {
  return (
    options.skipNonWorktreeDefaultMainDiffStats === true &&
    !session.state.is_worktree &&
    isDefaultMainComparisonRef(diffBase)
  );
}
...
export async function computeDiffStatsAsync(
  ...
  try {
    ...
    if (shouldSkipNonWorktreeDefaultMainDiffStats(session, diffBase, options)) {
...
export function recomputeDiffIfDirty(session: SessionDiffRefreshLike, deps: RecomputeDiffIfDirtyDeps): void {
  ...
  computeDiffStatsAsync(session, {
    skipNonWorktreeDefaultMainDiffStats: deps.isHeavyRepoModeEnabled?.() === true,
  })
    .then((didRun) => {
      if (!didRun) return;
...
async function runWorktreeGitStateRefreshForSnapshot(
  ...
  const didRun = shouldRefreshDiff
