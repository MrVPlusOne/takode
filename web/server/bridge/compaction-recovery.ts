...
3. Key rules:
   ...
   - Run \`memory recall "<current task or quest terms>"\` when prior memory may affect the current decision; read relevant \`current/\` files before continuing
...
4. Run \`memory recall "<current task or quest terms>"\` when prior memory may affect the task; read relevant \`current/\` files before continuing
