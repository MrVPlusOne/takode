...
export function buildSessionNavigationProjectionValue<TSubscriber>(
  ...
  return {
    ...
    topology: {
      treeGroupId: boundedNullableText(
        state.treeGroupId ?? launcherInfo?.treeGroupId,
        SESSION_NAVIGATION_ID_MAX_LENGTH,
      ),
    ...
    lifecycle: {
      sdkState: sdkStateFor(session, launcherInfo, cliConnected),
      status: deps.getSessionStatus(session.id),
      cliConnected,
      idleKilled: launcherInfo?.killedByIdleManager === true,
    ...
    quest: {
      ...
      claimedQuestVerificationInboxUnread:
        typeof state.claimedQuestVerificationInboxUnread === "boolean"
          ? state.claimedQuestVerificationInboxUnread
          : null,
    ...
    git: {
      branch: boundedText(state.git_branch),
      defaultBranch: boundedText(state.git_default_branch),
      diffBaseBranch: boundedText(state.diff_base_branch),
      ahead: nonNegativeInteger(state.git_ahead),
      behind: nonNegativeInteger(state.git_behind),
      linesAdded: nonNegativeInteger(state.total_lines_added),
      linesRemoved: nonNegativeInteger(state.total_lines_removed),
      diffStatsSkippedReason: boundedNullableText(state.diff_stats_skipped_reason),
      statusRefreshedAt: nonNegativeNullableNumber(state.git_status_refreshed_at),
      statusRefreshError: boundedNullableText(state.git_status_refresh_error),
    },
    detail: {
      ...
      configuredContextWindow: configuredWindow,
