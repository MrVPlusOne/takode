          ...
          while (Date.now() - start < maxWait) {
            ...
            if (info && (info.state === "connected" || info.state === "running")) {
              wsBridge.injectUserMessage(session.sessionId,
                ...
                `When speaking directly to the human, end your message with \`@to(user)\`. Internal coordination updates should end with \`@to(self)\`. Every leader message with text must end with one of these suffixes.\n\n` +
