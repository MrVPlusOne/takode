...
3. Key rules:
   ...
   - Scoped user-decision stop: if the summary or notifications show unresolved user decisions or \`needs-input\` prompts, do not advance the affected thread, quest, or board row, and do not answer on the user's behalf until the decision is resolved. Continue unrelated dispatch, quests, and herd events unless the prompt explicitly concerns global orchestration, worker-slot scheduling, shared resource safety, or another cross-quest dependency. If the user explicitly asks to defer, skip, or leave one prompt unresolved and continue unrelated work, comply under that same exception model
