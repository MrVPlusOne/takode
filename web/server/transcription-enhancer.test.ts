...
const { trunc, extractAssistantText, isAutoGeneratedName, isOrchestratorNoise, MAX_TURNS, MIN_WORDS_FOR_ENHANCEMENT, HALLUCINATION_LENGTH_RATIO, STT_PROMPT_MAX_CHARS, MAX_STT_SESSION_NAMES } = _testHelpers;
...
describe("isAutoGeneratedName", () => {
  it("detects auto-generated names (Adjective Noun pattern)", () => {
    expect(isAutoGeneratedName("Swift Falcon")).toBe(true);
    expect(isAutoGeneratedName("Calm River")).toBe(true);
    expect(isAutoGeneratedName("Verdant Lotus")).toBe(true);
    expect(isAutoGeneratedName("Keen Mesa")).toBe(true);
  });

  it("does not flag meaningful session names", () => {
    expect(isAutoGeneratedName("Fix sidebar layout")).toBe(false);
    expect(isAutoGeneratedName("Add voice transcription")).toBe(false);
    expect(isAutoGeneratedName("Debug WsBridge reconnect")).toBe(false);
  });
});
...
describe("filterSessionNames", () => {
  it("filters out auto-generated names", () => {
    const result = filterSessionNames(
      ["Swift Falcon", "Fix sidebar layout", "Calm River", "Add dark mode"],
      20,
    );
    expect(result).toEqual(["Fix sidebar layout", "Add dark mode"]);
  });

  it("deduplicates names", () => {
    const result = filterSessionNames(
      ["Fix sidebar", "Fix sidebar", "Add dark mode"],
      20,
    );
    expect(result).toEqual(["Fix sidebar", "Add dark mode"]);
  });

  it("filters out Takode placeholder name", () => {
    const result = filterSessionNames(
      ["Takode", "Fix sidebar", "Takode", "Add dark mode"],
      20,
    );
    expect(result).toEqual(["Fix sidebar", "Add dark mode"]);
  });
  ...
  it("handles empty and null-ish entries", () => {
    const result = filterSessionNames(["", "Fix sidebar", ""], 20);
    expect(result).toEqual(["Fix sidebar"]);
  });
});
...
describe("buildSttPrompt", () => {
  ...
  it("includes meaningful session name with Session: label", () => {
  ...
  it("omits auto-generated session names", () => {
    // Auto-generated names like "Swift Falcon" carry no vocabulary signal
    const prompt = buildSttPrompt({ sessionName: "Swift Falcon" });
    expect(prompt).toBe("");
  });

  it("shows other session names with Sessions: label, filtering auto-generated ones", () => {
    const prompt = buildSttPrompt({
      sessionName: "Debug voice input",
      activeSessionNames: [
        "Fix sidebar layout",
        "Swift Falcon",      // auto-generated, should be filtered
        "Calm River",        // auto-generated, should be filtered
        "Add dark mode",
        "Takode",            // placeholder, should be filtered
      ],
    });
    expect(prompt).toContain("Session: Debug voice input");
    expect(prompt).toContain("Sessions: Fix sidebar layout, Add dark mode");
    expect(prompt).not.toContain("Swift Falcon");
    expect(prompt).not.toContain("Calm River");
    expect(prompt).not.toContain("Takode");
  });
  ...
  it("fills in priority order: tasks > session > sessions > composer > conversation", () => {
    ...
    expect(lines[1]).toMatch(/^Session: Debug session/);
    expect(lines[2]).toMatch(/^Sessions: Other session/);
  ...
  it("limits other session names to MAX_STT_SESSION_NAMES", () => {
    // Generate more session names than the limit
    const manyNames = Array.from({ length: 30 }, (_, i) => `Session task ${i}`);
    const prompt = buildSttPrompt({ activeSessionNames: manyNames });
    // Count how many made it through
    const sessionsLine = prompt.split("\n").find((l) => l.startsWith("Sessions:"));
    expect(sessionsLine).toBeDefined();
    const names = sessionsLine!.replace("Sessions: ", "").split(", ");
    expect(names.length).toBeLessThanOrEqual(MAX_STT_SESSION_NAMES);
  });
