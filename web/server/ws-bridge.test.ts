...
// ─── Silent compaction detection (token-drop heuristic) ─────────────────────

describe("Silent compaction detection", () => {
  ...
  // between successive turns. handleResultMessage detects this and synthesizes
  // a compact_marker + compact_boundary broadcast.

  /** Helper: emit a result with usage through an SDK adapter. */
  function emitSdkResult(adapter: ReturnType<typeof makeClaudeSdkAdapterMock>, inputTokens: number, uuid: string) {
    adapter.emitBrowserMessage({
      type: "result",
      subtype: "success",
      is_error: false,
  ...
  it("detects silent compaction from result.usage token drop", () => {
    // Simulates two turns where the second has >50% fewer input_tokens than
    // the first (e.g., CLI silently compacted between turns). Verifies the
    // bridge synthesizes a compact_marker and broadcasts compact_boundary.
    const adapter = makeClaudeSdkAdapterMock();
    bridge.attachClaudeSdkAdapter("s1", adapter as any);
    adapter.emitBrowserMessage({
      type: "session_init",
    ...
    // Turn 1: high token count
    emitSdkResult(adapter, 150_000, "result-1");
    expect(session.lastResultInputTokens).toBe(150_000);

    browser.send.mockClear();
    const spy = vi.spyOn(bridge, "emitTakodeEvent");

    // Turn 2: token count drops >50% -- silent compaction
    emitSdkResult(adapter, 30_000, "result-2");

    expect(session.lastResultInputTokens).toBe(30_000);
    expect(session.compactedDuringTurn).toBe(true);

    // compact_marker should be in history (before the result)
    const markers = session.messageHistory.filter((m) => m.type === "compact_marker");
    expect(markers).toHaveLength(1);
    const marker = markers[0] as any;
    expect(marker.trigger).toBe("auto");
    expect(marker.preTokens).toBe(150_000);

    // Browser should receive compact_boundary
    const calls = browser.send.mock.calls.map(([arg]: [string]) => JSON.parse(arg));
    const compactMsg = calls.find((m: any) => m.type === "compact_boundary");
    expect(compactMsg).toBeTruthy();
    expect(compactMsg.trigger).toBe("auto");
    expect(compactMsg.preTokens).toBe(150_000);

    // Takode events should fire
    const started = spy.mock.calls.filter(([, e]) => e === "compaction_started");
    const finished = spy.mock.calls.filter(([, e]) => e === "compaction_finished");
    expect(started).toHaveLength(1);
    expect(finished).toHaveLength(1);
  });

  it("does not false-positive on small token counts", () => {
    // Even with >50% drop, if the previous count is below the 50K threshold,
    // it's likely just a short session or subagent -- not compaction.
    const adapter = makeClaudeSdkAdapterMock();
    bridge.attachClaudeSdkAdapter("s1", adapter as any);
    adapter.emitBrowserMessage({
      type: "session_init",
    ...
    emitSdkResult(adapter, 30_000, "r1"); // below 50K threshold
    emitSdkResult(adapter, 10_000, "r2"); // 67% drop, but prev was below threshold
  ...
  it("does not false-positive on normal token growth", () => {
    // Increasing tokens between turns is normal -- no compaction detected.
    const adapter = makeClaudeSdkAdapterMock();
    bridge.attachClaudeSdkAdapter("s1", adapter as any);
    adapter.emitBrowserMessage({
      type: "session_init",
    ...
    emitSdkResult(adapter, 80_000, "r1");
    emitSdkResult(adapter, 120_000, "r2");

    const markers = session.messageHistory.filter((m) => m.type === "compact_marker");
    expect(markers).toHaveLength(0);
  });

  it("updates context_used_percent on silent compaction detection", () => {
    // The pre-compaction token count is used to compute context_used_percent
    // (how full the context was before compaction) and broadcast as session_update.
    const adapter = makeClaudeSdkAdapterMock();
    bridge.attachClaudeSdkAdapter("s1", adapter as any);
    adapter.emitBrowserMessage({
      type: "session_init",
    ...
    // Turn 1: 180K tokens (90% of 200K context)
    emitSdkResult(adapter, 180_000, "r1");
    browser.send.mockClear();

    // Turn 2: silent compaction
    emitSdkResult(adapter, 20_000, "r2");

    // context_used_percent should reflect pre-compaction usage (180K/200K = 90%)
    // Note: the result handler also updates context_used_percent from the result
    // itself, so we check that the session_update broadcast included the compaction
    // context percent at some point.
    const calls = browser.send.mock.calls.map(([arg]: [string]) => JSON.parse(arg));
    const compactBoundary = calls.find((m: any) => m.type === "compact_boundary");
    expect(compactBoundary).toBeTruthy();
    expect(compactBoundary.preTokens).toBe(180_000);
  });

  it("does not double-fire when explicit compaction already handled the turn", () => {
    // If the CLI emits system.status:compacting (explicit path) AND the result
    // shows a token drop, we must not create a second compact_marker. The
    // compactedDuringTurn flag guards against this.
    const adapter = makeClaudeSdkAdapterMock();
    bridge.attachClaudeSdkAdapter("s1", adapter as any);
    adapter.emitBrowserMessage({
      type: "session_init",
    ...
    // Turn 1: set baseline
    emitSdkResult(adapter, 150_000, "r1");

    // Explicit compaction fires via status_change (sets compactedDuringTurn)
    adapter.emitBrowserMessage({ type: "status_change", status: "compacting" });
    adapter.emitBrowserMessage({ type: "status_change", status: null });

    // Turn 2: token drop would also trigger silent detection, but the
    // explicit path already set compactedDuringTurn = true
    emitSdkResult(adapter, 30_000, "r2");

    // Only one compact_marker (from the explicit status_change path)
    const markers = session.messageHistory.filter((m) => m.type === "compact_marker");
    expect(markers).toHaveLength(1);
  });
});
