...
describe("Codex startup relaunch recovery", () => {
  it("re-dispatches an initially queued Codex turn after attach-then-disconnect during startup", async () => {
    // Regression for q-25 reviewer startup: the first process can attach,
    // start the queued review dispatch, then die before producing a result
    // (for example after a fatal auth refresh failure). The queued startup
    // turn must survive that restart and be re-dispatched on the replacement
    // adapter's session_meta/resume path.
    const sid = "s-startup-auth-restart";
    const relaunchCb = vi.fn();
    const launcherInfo = { state: "starting", killedByIdleManager: false };
    bridge.onCLIRelaunchNeededCallback(relaunchCb);
    bridge.setLauncher({
      touchActivity: vi.fn(),
      getSession: vi.fn(() => launcherInfo),
      getSessionNum: vi.fn(() => 107),
    } as any);
    ...
    await bridge.handleBrowserMessage(
      ...
      JSON.stringify({
        type: "user_message",
        content: "Skeptic review session #95 / quest q-25. Read changes in session #95 around messages [647]-[670].",
      }),
    );
    await Promise.resolve();

    const queuedSession = bridge.getSession(sid)!;
    expect(queuedSession.pendingCodexInputs).toHaveLength(1);
    expect(getPendingCodexTurn(queuedSession)).toMatchObject({
      status: "queued",
      userContent: "Skeptic review session #95 / quest q-25. Read changes in session #95 around messages [647]-[670].",
    });

    const adapter1 = makeCodexAdapterMock();
    bridge.attachCodexAdapter(sid, adapter1 as any);
    adapter1.emitSessionMeta({
      cliSessionId: "thread-startup-auth-restart",
      model: "gpt-5.4",
      cwd: "/repo",
    });
    adapter1.emitTurnStarted("turn-startup-auth-1");

    expect(queuedSession.pendingCodexInputs).toHaveLength(0);
    expect(getPendingCodexTurn(queuedSession)).toMatchObject({
      status: "backend_acknowledged",
      turnId: "turn-startup-auth-1",
    });

    relaunchCb.mockClear();
    launcherInfo.state = "connected";
    adapter1.emitDisconnect("turn-startup-auth-1");
    ...
    adapter2.emitSessionMeta({
      ...
      cwd: "/repo",
      resumeSnapshot: {
        threadId: "thread-startup-auth-restart",
        threadStatus: "idle",
        turnCount: 1,
        lastTurn: null,
        turns: [],
    ...
    expect(adapter2.sendBrowserMessage).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        inputs: expect.arrayContaining([
          expect.objectContaining({
            content:
              "Skeptic review session #95 / quest q-25. Read changes in session #95 around messages [647]-[670].",
          }),
        ]),
      }),
    );
