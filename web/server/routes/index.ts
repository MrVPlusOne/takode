...
export function buildOrchestratorSystemPrompt(backend: "claude" | "codex" | "claude-sdk"): string {
  ...
  return (
    ...
    `- If a worker/reviewer question exposes ambiguity you cannot resolve, publish the detailed question with \`takode user-message --text-file -\`, then call \`takode notify needs-input\` after that user-visible text exists, optionally with one to three short \`--suggest <answer>\` choices for obvious answers, and do not keep advancing that quest until it is resolved.\n` +
