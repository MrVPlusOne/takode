...
export function buildOrchestratorSystemPrompt(backend: "claude" | "codex" | "claude-sdk"): string {
  ...
  return (
    ...
    `**Key disciplines**:\n` +
    `- Visible leader messages must declare both destination and role: use \`[thread:main:C]\` / \`[thread:q-N:C]\` for commentary and \`[thread:main:A:u1]\` / \`[thread:q-N:A:u1,u2]\` for a self-contained answer to explicit earlier user-message IDs. Shell commands remain commentary and start with \`# thread:main\` or \`# thread:q-N\`.\n` +
