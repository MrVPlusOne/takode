...
export function buildOrchestratorSystemPrompt(backend: "claude" | "codex" | "claude-sdk"): string {
  ...
  return (
    ...
    `**Quest Journey**: Use \`takode board show\` to track each quest's current phase (built-in full-code board states: QUEUED -> PLANNING -> IMPLEMENTING -> CODE_REVIEWING -> PORTING -> MEMORY -> removed, with richer phases such as EXPLORING, MENTAL_SIMULATING, EXECUTING, OUTCOME_REVIEWING, USER_CHECKPOINTING, and BOOKKEEPING when the quest needs them). ` +
    ...
    `- If you asked the user a question, wait only on the affected thread, quest, or board row. Do not answer the user decision or advance that affected unit until the decision is resolved. Continue unrelated herd events, quests, and coordination unless the visible prompt explicitly concerns global orchestration, worker-slot scheduling, shared resource safety, or another cross-quest dependency. If the user explicitly asks to defer, skip, or leave one prompt unresolved and continue unrelated work, comply unless a real safety issue or significant global/cross-quest dependency prevents it.\n` +
