...
description: "Cross-session orchestration for Takode orchestrator sessions. Use when you need to coordinate multiple agent sessions: peeking at session activity, reading messages, sending instructions to workers, or listing active sessions. Only available when TAKODE_ROLE=orchestrator is set. Triggers: 'check on workers', 'send to session', 'orchestrate', 'coordinate agents', 'list sessions', 'peek at session', 'what are my sessions doing'."
---
...
### 2. React to auto-delivered herd events

There is no `takode watch` command. Herd events are delivered automatically as user messages in your orchestrator session when you go idle.

When you receive a `[Herd]` summary:
- On `turn_end`: inspect with `takode peek`, then send follow-up work or report completion
- On `session_error`: inspect recent turns and send recovery steps
- On `permission_request`: coordinate with the human (tool permissions are human-approved in UI)
- On `user_message`: incorporate the latest instruction/priority updates
...
4. **React to herd event messages** — events are pushed automatically while avoiding poll loops
...
- **Keep event handling tight.** Process each herd summary quickly, decide next actions, then return to coordination.
