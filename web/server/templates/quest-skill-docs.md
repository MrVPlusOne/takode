...
After successful quest creation, refinement, or dispatch, include a lightweight non-blocking thread reminder when prior discussion may belong to the quest thread: Thread reminder: attach any prior messages that clearly belong to this quest to [q-N](quest:q-N) with `takode thread attach`.

For Journey notes, standard tracked-code phases are self-explanatory by default: `alignment`, `implement`, `code-review`, and `port` only need notes for unusual phase-specific work. Non-standard phases such as `explore`, `execute`, `outcome-review`, `mental-simulation`, and `bookkeeping` should be explained concisely with why the phase is needed and what evidence, scenario, outcome, or durable state it covers. Standard phases are recommended defaults, not mandates; user overrides win.
