...
quest outcome show <id> [--json]                               Inspect preserved legacy Outcome data (read-only)
...
To browse a known historical range ending at a recorded commit, add `--range <base-sha>..<recorded-tip-sha>`. Supply the base from explicit source evidence; the command does not infer a delivery baseline. It verifies ancestry and the complete range in the delivery's recorded repository and returns fixed range links for up to 200 commits, optionally selected with `--commits`. These are individual first-parent comparisons, not aggregate range totals or newly recorded deliveries. Range links preserve both full endpoints and keep the original delivery metadata, saved counts, and ordinary fixed links unchanged. Missing objects, incomplete shallow history, invalid membership, and oversized ranges fail explicitly; no repository fallback or remote fetch occurs.

For already-authorized independent publication outside the inherited target, the assigned leader uses `takode board approve-delivery-target` with the complete ordered final Work `commitShas` separately from published `refs` heads; the worker supplies that exact commit list with `--delivery-target <approval-id>` to guarded delivery recording or Work-to-Memory. A head-only approval cannot stand in for the complete commit set. Read the independent-target section of the `/port-changes` port-tracking reference for selection, verification and historical-approval handling. This preserves normal ownership/evidence guards and never requires repeating a push or moving an unrelated branch.
