...
- Run the required self-checks yourself before handoff. For tracked code/test changes, the current full automated gate is `cd web && bun run typecheck`, `cd web && bun run test`, and `cd web && bun run format:check`.
