...
6. **Submit**: `quest complete q-N --items "..."` for simple inline lists, or `quest complete q-N --items-file <path>` / `--items-file -` for comma-heavy or copied verification text. Only list human-checkable acceptance items (UI appearance, UX feel, edge cases needing judgment). Keep implementation details, synced SHAs, port status, and automated verification results in the consolidated `Summary:` feedback comment and structured `--commit/--commits` metadata. Keep checklist items concise — one short sentence each, scannable at a glance.
