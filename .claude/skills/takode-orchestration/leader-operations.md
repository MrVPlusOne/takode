...
- **Don't echo board state as prose.** `takode board set/advance` already displays the board in the terminal with a special UI. Never repeat that information as markdown tables or summaries in your message -- it wastes tokens and clutters the conversation. Just run the command and move on.
