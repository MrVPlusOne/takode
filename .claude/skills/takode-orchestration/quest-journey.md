...
Takode memory is a Git-tracked Markdown repo scoped to the current server/session space. Normal `memory` commands auto-create the repo at `~/.companion/memory/<serverSlug>` when needed, so agents do not need a separate init step. It is a shared aid for durable state, not a hidden instruction channel. Leaders and workers should use visible reads, usually `memory catalog show` followed by direct Markdown file inspection, when prior memory could change dispatch, Alignment, compaction recovery, Bookkeeping, Port, or final handoff decisions. After compaction or low-confidence recovery, use `memory catalog`, `memory repo path`, and `memory --help` to rediscover the repo and command surface.
