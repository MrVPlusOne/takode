...
- **Use commentary and answer roles deliberately.** Commentary is chronological coordination or process detail: dispatch, progress, recovery, verification, waiting, User Checkpoints, and asynchronous Memory work. It cannot satisfy answer coverage. An answer is a polished, self-contained response to the explicit earlier IDs in its marker, independently of whether the whole thread is Ready.
- **Answer coverage and supersession are explicit.** Every direct user message delivered to a leader has a source-envelope ID such as `u17`. A valid `:A:<ids>` answer covers only the listed earlier IDs after server validation. A later valid answer supersedes earlier answers for the IDs it repeats while append-only history keeps every row. Thread Ready remains separate and is rejected until all currently thread-owned direct user IDs have valid coverage.
...
- **Answer:** start polished self-contained prose with `[thread:main:A:u1]` or `[thread:q-N:A:u1,u2]`, listing every earlier user-message ID that prose answers. Use `takode read <leader-session> <user-message-id>` to retrieve an exact earlier message when needed.
- **Supersession and Ready:** a later answer supersedes earlier answers only for IDs it repeats. New or reassigned user messages remain pending until explicitly answered. Thread Ready requires complete current ownership-aware coverage and rejects invalid attempts with a concise ID reminder.
