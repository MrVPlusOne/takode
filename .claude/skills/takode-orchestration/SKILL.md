...
- **Final Memory is mandatory.** Every completed non-cancelled quest must have final Memory closure, a final debrief, and a debrief TLDR. A quest in `MEMORY` is downstream-unblocking because substantive work is accepted and synced when applicable, but completion is incomplete until Memory settles durable state and the memory statement.
