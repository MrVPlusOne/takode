...
- **Commentary:** start visible operational or in-progress text with `[thread:main:C]` or `[thread:q-N:C]`. Commentary remains in expanded chronology and cannot satisfy answer coverage.
...
- **Coverage and Ready:** a later answer becomes current coverage authority only for IDs it repeats; all valid answers remain chronologically visible. New or reassigned user messages remain pending until explicitly answered. Thread Ready requires complete current ownership-aware coverage and rejects invalid attempts with a concise ID reminder.
