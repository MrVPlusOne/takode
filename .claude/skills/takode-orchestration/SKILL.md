...
- **Final Memory is mandatory.** Every completed non-cancelled quest must have final Memory closure, final User review check settlement, a final debrief, and a debrief TLDR. A quest in `MEMORY` is downstream-unblocking because substantive work is accepted and synced when applicable, but completion is incomplete until Memory settles durable state, User review checks, and the memory statement.
