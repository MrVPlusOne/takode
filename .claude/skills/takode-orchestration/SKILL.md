...
- **System-interrupted worker events can be provisional without shrinking Work.** If a `turn_end (⊘)` event says `recovery pending`, or the worker still appears connected/generating after a stuck-watchdog interruption, inspect `takode info`/`takode peek`/`takode scan` once and allow one short verification window. If progress continues, let the same turn run. If no progress is evident, interrupt the stale turn and resume the full remaining authorized Work envelope instead of authorizing one substep. Treat status requests as updates, not implicit pauses. Resume from durable evidence, verify rather than repeat completed external side effects, and refresh volatile preconditions inside the same envelope. Stop or narrow only at a genuine authority, safety, destructive/external-action, shared-resource/lease, launch, or changed-state gate; never bypass Takode's exact-once replay proof or recovery suppression.
