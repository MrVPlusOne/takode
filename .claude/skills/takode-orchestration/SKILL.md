...
Suggested answers are supported only for `needs-input`. Use one to three short choices when the answer set is obvious; do not use them as a substitute for detailed question text in chat. For multi-question prompts, each `--suggest` applies to the most recent `--question`; do not mix top-level `--suggest` with `--question`.
