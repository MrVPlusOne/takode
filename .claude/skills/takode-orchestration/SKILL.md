...
- **Quest completion responses are final responses.** When telling the user a quest is complete, publish one polished self-contained answer beginning with `[thread:q-N:F]` and lead with what changed or was decided, why it matters, and any real next action or residual risk. Do not lead with Journey bookkeeping. Routine internals such as raw commit hashes, empty User review checks, final debrief metadata status, no-op memory statements, command lists, or routine verification belong in structured metadata, phase notes, or debrief bodies unless directly useful to the user. If quiz metadata exists or was updated, keep `{[(Quest Quiz: q-N)]}` out of the final-response prose and emit it as a separate `[thread:q-N:C]` assistant directive before any Thread Ready marker.
- **Route every visible leader message with a compact destination-and-role marker.** Use `[thread:main:C]` or `[thread:q-N:C]` for commentary and `[thread:main:F]` or `[thread:q-N:F]` for a final response. The marker is stripped from rendering and becomes server-authored route and role metadata. If one assistant output intentionally covers multiple thread tabs or roles, keep the first marker on the first line, then put a standalone `---` line immediately before each later role-bearing marker. Shell/terminal commands remain commentary and use `# thread:main` or `# thread:q-N` as their first non-empty line.
- **Use commentary and final-response roles deliberately.** Commentary is chronological coordination or process detail: dispatch, progress, recovery, verification, waiting, User Checkpoints, and asynchronous Memory work. It cannot satisfy a pending response requirement. A final response is the polished, self-contained direct answer that should remain useful after the request or quest is finished.
- **Pending response batches and supersession are server-owned.** A valid `:F` message automatically covers the current pending same-thread user batch; leaders never choose or copy user-message IDs, batch tokens, response IDs, or revision IDs. Another `:F` before newer user input supersedes the prior current final while append-only history keeps both. Any newer direct user message creates a new pending requirement, and Thread Ready is rejected with a reminder until a later valid final response covers it.
- **Do not use `takode user-message` as the publishing path.** It remains deprecated compatibility only. Use ordinary role-bearing routed assistant output and `takode notify` when notification state or suggested answers are needed.
...
- **Commentary:** start visible operational or in-progress text with `[thread:main:C]` or `[thread:q-N:C]`. Commentary remains in expanded chronology and cannot satisfy a pending response batch.
- **Final response:** start the polished self-contained direct answer with `[thread:main:F]` or `[thread:q-N:F]`. Takode assigns the current pending same-thread user batch automatically; never write message IDs or internal response identifiers.
- **Supersession and Ready:** another final response before newer user input supersedes the prior current final. A newer user message requires a later final before Thread Ready can succeed. Invalid Ready attempts trigger a reminder containing the pending request context.
...
- **Shell commands:** use `# thread:main` or `# thread:q-N` as the first non-empty command line. Shell commands are commentary; they do not publish a final response.
...
Deprecated compatibility publisher for older leader sessions. New leader workflow uses role-bearing routed responses: `[thread:main:C]` / `[thread:q-N:C]` for commentary and `[thread:main:F]` / `[thread:q-N:F]` for final responses. Do not use `takode user-message` as the core thread publishing path.
