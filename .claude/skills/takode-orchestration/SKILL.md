...
- **Use `takode notify` at these moments:** `needs-input` when a quest/worker is blocked on a user decision or a new quest is ready for dispatch approval; `review` when a quest completes the full journey and is ported.
