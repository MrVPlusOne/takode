...
- **Keep Work ownership explicit.** The worker doing the job claims the quest, owns technical Work, and performs the routine guarded Work -> Memory transition. The leader owns user intent and corrections, approved scope/Journey, entry into Work, checkpoint state, and genuine Journey revisions; the leader does not become the technical Work owner.
...
- **Report accepted Work before Memory closure.** When the guarded Work transition reaches `MEMORY`, promptly tell the user the main accepted answer, finding, or outcome from the Work note, then send final Memory to the normal same worker without waiting for closure. Keep the quest technically open in Memory, but treat it as downstream-unblocking unless a dependent explicitly needs a Memory-produced output. Ordinary read-only follow-ups during Memory use accepted evidence and do not reopen the quest; substantive new work follows the normal rework lifecycle.
...
|-------|---------|--------|
| `turn_end (✓)` | Worker completed successfully | Peek at output and inspect the board phase. If Work just entered `MEMORY`, report the accepted Work outcome and send final Memory without waiting for closure. In `PLANNING`, this may contain a plain-text alignment read-in that should be reviewed and answered with normal `takode send` rather than `takode answer` |
