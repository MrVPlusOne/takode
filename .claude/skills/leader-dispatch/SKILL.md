...
Recovery does not narrow that authorization. After a system, transport, provider-stream, compaction, or leader-recycle interruption, reconstruct the latest durable state and send one continuation that restores the full remaining authorized Work envelope. Do not split implementation, validation, commit, sync/push, approved preparation, or read-only preflight into leader-approved micro-handoffs merely because the previous turn disconnected. Treat a status request as an update, not an implicit pause. For `recovery pending`, inspect progress once and allow one short verification window; if progress continues, let the turn run, and if no progress is evident, interrupt the stale turn and resume the same full remaining envelope. Refresh volatile preconditions inside the envelope, verify rather than repeat completed side effects, and stop or narrow only at a genuine authority, safety, destructive/external-action, shared-resource/lease, launch, or changed-precondition gate. Existing exact-once replay proof and recovery-suppression rules remain authoritative.
