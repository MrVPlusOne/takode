...
**Every completed non-cancelled quest needs final Memory.** Completion without Memory closure, final debrief metadata, debrief TLDR metadata, and one memory statement is incomplete. A quest in `MEMORY` is downstream-unblocking because substantive work is accepted and synced when applicable, but the row remains open until Memory finishes.

**Every memory-relevant phase needs a memory statement.** If durable shared facts, live coordination state, accepted preferences/decisions, source digests, or external artifact manifests should be captured, the phase actor should update file-based memory under the repo-level lock or defer that work deliberately. Reports should include exactly one of `memory updated: <commit>`, `memory update deferred: <reason or curator>`, or `memory update not needed: <reason>`.
...
- **Porting**: send a separate, explicit `/port-changes` instruction only after reviewer ACCEPT. The Port assignee brief owns the standard report shape: dedicated `Synced SHAs: sha1,sha2`, post-port verification status, memory statement when relevant, and accepted-state context final Memory will need. Your handoff should add only context-dependent deltas: accepted commits or artifacts to sync, required extra verification, known port risks, and whether durable memory writing is explicitly assigned. If required post-port verification fails, dispatch a suitable worker to fix `main` immediately rather than waiting.
...
- **Memory**: use this mandatory final phase for non-project-tracked durable-state closure, including final debrief metadata, memory consistency checks, memory writes or deferrals, cleanup, external durable-state records, and follow-up routing. Memory must not edit tracked project files.
- **Bookkeeping**: use this compatibility phase for cross-phase or external durable state beyond normal phase notes when it is not final Memory closure, such as consolidated summaries, verification checklist reconciliation, external docs or links, superseded facts, notification cleanup, thread cleanup, or shared-state updates. Do not dispatch Bookkeeping just to repeat normal phase documentation.
...
Use the Port assignee brief for standard post-port verification, memory-statement, and accepted-state context final Memory will need. Leader-specific deltas for this port: <accepted refs, extra verification, known port risks, explicit memory-writing assignment if any>.
```
