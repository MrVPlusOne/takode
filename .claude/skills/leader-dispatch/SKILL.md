...
**Every completed non-cancelled quest needs final Memory.** Completion without Memory closure, final User review check settlement, final debrief metadata, debrief TLDR metadata, and one memory statement is incomplete. A quest in `MEMORY` is downstream-unblocking because substantive work is accepted and synced when applicable, but the row remains open until Memory finishes.
