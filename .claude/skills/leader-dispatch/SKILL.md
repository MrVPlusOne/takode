...
- **Porting**: send a separate, explicit `/port-changes` instruction only after reviewer ACCEPT. The Port assignee brief owns the standard report shape: dedicated `Synced SHAs: sha1,sha2`, post-port verification status, memory statement when relevant, and final debrief metadata status or draft. Your handoff should add only context-dependent deltas: accepted commits or artifacts to sync, required extra verification, known port risks, whether the port worker should complete the quest or only draft debrief metadata, and whether durable memory writing is explicitly assigned. If required post-port verification fails, dispatch a suitable worker to fix `main` immediately rather than waiting.
...
Use the Port assignee brief for standard post-port verification, memory-statement, and final debrief metadata requirements. Leader-specific deltas for this port: <accepted refs, extra verification, completion owner, explicit memory-writing assignment if any>.
```
