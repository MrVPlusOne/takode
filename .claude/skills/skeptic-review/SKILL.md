...
- If the task, scope, or claimed behavior is ambiguous enough that you cannot review confidently, ask the leader in plain text first, then call `takode notify needs-input` with a short summary, optionally using one to three short `--suggest <answer>` choices for obvious answers, and stop instead of guessing.
