...
If the scope is nominally clear but you still see real misunderstanding risk, ask the leader immediately in plain text first, then call `takode notify needs-input` with a short summary, optionally using short `--suggest <answer>` shortcuts for obvious answers, and stop until you get an answer.
