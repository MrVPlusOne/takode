...
- [ ] **Fix permission prompts showing with Ask toggle OFF** — Two bugs caused permission prompts to appear even with Ask toggle OFF: (1) the post-plan mode transition was inverted (askPermission=false resolved to "acceptEdits" instead of "bypassPermissions"), and (2) the server always forwarded CLI permission requests to the browser regardless of askPermission state. Now the server auto-approves tool permissions when askPermission=false (except ExitPlanMode, which always requires explicit approval). Verify: create a new session with Ask toggle OFF in both Plan and Agent modes. In Plan mode, approve the plan and confirm subsequent tool calls execute without permission prompts. In Agent mode, confirm tool calls never show permission prompts. Also verify that ExitPlanMode still shows the plan approval UI regardless of Ask toggle state.
